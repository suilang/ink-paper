...
**外接显示器 + 任务控制（Mission Control）时，底层窗口模式壁纸可能闪一下。** 这是 macOS 在多屏合成任务控制界面时的系统行为，目前无解。

临时建议（应用支持**按显示器分别设置**壁纸）：

- **仅设置内屏（笔记本屏幕）**，外接屏不启用 / 不铺底层窗口；或
- **仅使用外接显示器**（合盖或只给外屏设图），避免内屏与外屏同时走底层窗口

系统壁纸模式不受此问题影响；仅在底层窗口兜底、且内屏与外接屏同时覆盖时更容易遇到。

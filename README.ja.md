...
**外付けディスプレイ使用時、ミッションコントロールに入るとオーバーレイ壁紙が一瞬ちらつくことがあります。** これは macOS がマルチディスプレイでミッションコントロールを合成する際のシステム側の挙動で、現状アプリ側の確実な回避策はありません。

暫定の使い方（**ディスプレイごとの壁紙設定**に対応しています）：

- **内蔵ディスプレイのみ**設定し、外付けにはオーバーレイを載せない；または
- **外付けディスプレイのみ**使う（クラムシェルや外付けだけに画像を設定し、内蔵と外付けの両方で同時にオーバーレイしない）

システム壁紙モードでは起きません。オーバーレイ退避で内蔵と外付けを同時に覆っているときに起きやすいです。
